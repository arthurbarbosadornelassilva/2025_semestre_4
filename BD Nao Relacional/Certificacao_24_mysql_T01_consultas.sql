...
*/
CREATE SCHEMA IF NOT EXISTS tti204 DEFAULT CHARACTER SET utf8mb4 ;
USE tti204 ;
...
fone_aluno NUMERIC(11)) 
ENGINE = InnoDB;

-- gatilho para validar sexo de aluno
DROP TRIGGER IF EXISTS check_aluno_sexo ;
DELIMITER $$
CREATE TRIGGER check_aluno_sexo BEFORE INSERT ON aluno
FOR EACH ROW
BEGIN
    IF NEW.Sexo_aluno NOT IN ('M', 'F') THEN
         SIGNAL SQLSTATE '45000'
        SET MESSAGE_TEXT = 'Valor não permitido para sexo de aluno';
    END IF;
END;
$$   
...
INSERT INTO aluno_modulo VALUES (1,'ICND1','2020-1',9.0,54,'ReproFalta');
...
-- erro proposital
-- INSERT INTO aluno_modulo VALUES (null,'ICND2','2020-2',6.0,100,'Aprovado' ); 
...
-- adicionando colunas em aluno
desc aluno ;
ALTER TABLE aluno ADD COLUMN situacao_aluno CHAR(15) NULL  ;
SELECT * FROM aluno ;
UPDATE aluno SET situacao_aluno = 'CURSANDO' ;
-- alterar a definição para NOT NULL
ALTER TABLE aluno MODIFY COLUMN situacao_aluno CHAR(15) NOT NULL ;
-- renomear uma coluna 
ALTER TABLE aluno RENAME COLUMN end_aluno TO endereco_aluno ;
-- renomear uma tabela
...
ALTER TABLE modulo ADD COLUMN qtde_aulas TINYINT , 
                   ADD COLUMN duracao_aula_min TINYINT ;
describe modulo ;
-- outra forma em outros SGBDS
-- ALTER TABLE modulo ADD COLUMN (qtde_aulas TINYINT , duracao_aula_min TINYINT)

-- excluindo novas colunas
ALTER TABLE modulo DROP COLUMN qtde_aulas,
                   DROP COLUMN duracao_aula_min ;
-- resumo ADD COLUMN, RENAME COLUMN, MODIFY COLUMN, DROP COLUMN 
-- tipo de dado de coluna
SELECT * FROM aluno_certificacao ;
ALTER TABLE aluno_certificacao MODIFY dt_matricula TIMESTAMP ;
-- atualizando dt_matricula com INTERVAL
UPDATE aluno_certificacao
SET dt_matricula = current_timestamp - INTERVAL '15' MINUTE 
WHERE num_matricula = 4 AND sigla_cert = 'CCENT' ;
-- definindo um valor padrão para uma coluna
ALTER TABLE aluno MODIFY COLUMN situacao_aluno CHAR(15) DEFAULT 'CURSANDO' ;

INSERT INTO aluno 
VALUES (default,'Caetano Veloso dos Reis','M','Rua Beta,20', 1998487766, default);
SELECT * FROM aluno ;

-- Revisão Consultas - SELECT 09-ago-2024
-- 1 - Conversão de tipos de dados - CAST
DESCRIBE aluno ;
SELECT CAST(num_matricula AS CHAR) AS Matricula, 
       CAST(fone_aluno AS CHAR) AS Fone, CAST('+123' AS SIGNED),
       CAST('2024-10-01 11:55:22' AS DATETIME)
FROM aluno ; 
-- 2 - Formatando e concatenando strings - CONCAT-> gruda uma string na outra 
SELECT CONCAT(nome_aluno, ' é aluno') AS Dados_aluno
FROM aluno ;

SELECT UPPER(nome_modulo), LOWER(descr_modulo)
FROM modulo ;
DESC modulo ;
SELECT CONCAT(CONCAT(UPPER(nome_modulo),  ' trata do assunto '), LOWER(descr_modulo)) AS 
"Informações do módulo", carga_horaria_modulo
FROM modulo
WHERE carga_horaria_modulo > 80 ;
-- 3 - Trabalhando com datas
SELECT now(), curdate() AS "Data atual", curtime() AS "Hora atual",
       current_date AS "Padrão SQL", current_timestamp AS "Padrão SQL",
       localtimestamp AS "Data hora" ;
-- 4 - Extraindo pedaços da data e contas com intervalo de tempo
SELECT EXTRACT(MONTH FROM current_date) AS Mês,
       EXTRACT(YEAR_MONTH FROM now()) AS Ano_Mês,
       EXTRACT(WEEK FROM curdate()) AS SEmana_ano, EXTRACT(MINUTE FROM Curtime()) AS Minuto,
       current_date + INTERVAL '2' MONTH ,
       current_timestamp - INTERVAL '2' HOUR - INTERVAL '55' MINUTE ;
-- 5 - Junção - JOIN
SELECT a.nome_aluno, m.nome_modulo, am.nota_final, m.carga_horaria_modulo, 
       am.resultado_final
FROM aluno a INNER JOIN aluno_modulo am
	ON ( a.num_matricula = am.num_matricula)
             INNER JOIN modulo m
	ON ( am.sigla_modulo = m.sigla_modulo)
WHERE m.carga_horaria_modulo BETWEEN 60 AND 100
AND UCASE(am.resultado_final) LIKE '%APROVADO%' ;
-- 6 - Junção - INNER JOIN com USING
SELECT a.nome_aluno, m.nome_modulo, am.nota_final, m.carga_horaria_modulo, 
       am.resultado_final
FROM aluno a INNER JOIN aluno_modulo am USING (num_matricula)
             INNER JOIN modulo m USING (sigla_modulo )
WHERE m.carga_horaria_modulo BETWEEN 60 AND 100
AND UCASE(am.resultado_final) LIKE '%APROVADO%' ;
-- 7 - Junção externa - diferença quem está na tabela A mas não está na tabela B
-- alunos que ainda não fizeram modulos
SELECT a.nome_aluno, a.num_matricula AS Matricula, am.num_matricula AS "Não fizeram"
FROM aluno a LEFT JOIN aluno_modulo am 
     ON ( a.num_matricula = am.num_matricula)
WHERE am.num_matricula IS NULL ;
-- 8 - Função de grupo - GROUP BY
-- Mostrar a qtde de alunos matriculados por certificação
SELECT c.nome_cert AS Certificacao, COUNT(*) AS Qtde_matriculados
FROM aluno a JOIN aluno_certificacao ac USING ( num_matricula)
             JOIN certificacao c USING (sigla_cert)
GROUP BY Certificacao
HAVING Qtde_matriculados >= 3 ;
-- 9 - agrupando também por mes-ano da matricula
SELECT c.nome_cert AS Certificacao, EXTRACT(YEAR_MONTH FROM ac.dt_matricula) AS Mes_ano,
       COUNT(*) AS Qtde_matriculados
FROM aluno a JOIN aluno_certificacao ac USING ( num_matricula)
             JOIN certificacao c USING (sigla_cert)
GROUP BY Certificacao, Mes_ano 
HAVING Qtde_matriculados >= 2 ;
